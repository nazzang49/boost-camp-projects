...
class Conv1dHeadConcat(nn.Module):
    '''A class as a head of PLM for convolutional process (concat)
...
class Conv1dHeadSum(nn.Module):
    '''A class as a head of PLM for convolutional process (broadcasting summation)
...
class LSTMConv1dHeadConcat(nn.Module):
    '''A class as a head of PLM for convolutional process (concat + lstm)
    ...
    def __init__(self, hidden_size):
        ...
        self.conv1 = nn.Conv1d(in_channels=hidden_size * 2, out_channels=256, kernel_size=1)
        self.conv2 = nn.Conv1d(in_channels=hidden_size * 2, out_channels=256, kernel_size=3, padding=1)
        self.conv3 = nn.Conv1d(in_channels=hidden_size * 2, out_channels=256, kernel_size=5, padding=2)
        self.dropout = nn.Dropout(p=0.3)
        self.fc = nn.Linear(in_features=256 * 3, out_features=2)
